...
*Nota di manutenzione (14/08/2026): aggiornate le dipendenze di build (`marked`, `mustache`) per correggere vulnerabilità di sicurezza note (ReDoS in `marked`, XSS in `mustache`); adeguato `render.js` alla nuova API di `marked`. Sostituito il tool `gh-pages-deploy` (abbandonato e già non funzionante) con il pacchetto mantenuto `gh-pages` per `npm run deploy`. Rimossa la cartella `javascripts/` (rainbow.js e altri script di syntax highlighting mai effettivamente caricati dalla pagina). Deploy ora automatico: una GitHub Action ricostruisce e ripubblica il sito ad ogni push su `master`. Nessuna modifica al contenuto del tutorial.*
